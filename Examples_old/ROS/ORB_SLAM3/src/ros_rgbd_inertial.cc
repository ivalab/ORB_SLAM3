...
public:
    ...
    void GrabRgbdInertial(const sensor_msgs::ImageConstPtr& msgRGB, const sensor_msgs::ImageConstPtr& msgD, const sensor_msgs::ImuConstPtr& msgImu);
...
void ImageGrabber::GrabRgbdInertial(const sensor_msgs::ImageConstPtr& msgRGB, const sensor_msgs::ImageConstPtr& msgD, const sensor_msgs::ImuConstPtr& msgImu) {
    if (dataset_.find(std::string("gazebo")) != std::string::npos) {
        // @NOTE (yanwei) throw the first few garbage images from gazebo
        static size_t skip_imgs = 0;
        if (skip_imgs < 10)
